...
import { User, FileText, Code, Briefcase, MessageSquare, Mail, Search, MoreHorizontal } from 'lucide-react'
...
export default function Taskbar({ activeApp, openApp, toggleStartMenu }: TaskbarProps) {
  const apps = [
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-gray-900 bg-opacity-80 backdrop-blur-md p-2 flex justify-center items-center space-x-2">
      ...
      {apps.map((app) => (
        <button
          key={app.name}
          className={`p-2 rounded-lg transition-colors text-green-500 ${activeApp === app.name ? 'bg-gray-800' : 'hover:bg-gray-800'}`}
